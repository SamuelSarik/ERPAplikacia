    ...
    public partial class MainMenuPage : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (success == "True")
            {
                ...
                OrderLabel.Text = "Vaša objednávka bola úspešne zaregistrovaná. Aktuálny stav môžete zobraziť kliknutím na tlačidlo Zobraziť objednávky.";
            }
            else if (success == "False")
            {
                OrderLabel.Visible = true;
                OrderLabel.Text = "Vaša objednávka zlyhala. Znova zadajte svoju objednávku.";
            }
            else
            {
        ...
        {
            Response.Redirect("EmployeePage.aspx?User_Id=" + userId + "&Authorisation=" + authorisation + "&User_Name=" + userName);
        }

        protected void ProductButton_Click(object sender, EventArgs e)
        {
            Response.Redirect("ProductPage.aspx?User_Id=" + userId + "&Authorisation=" + authorisation + "&User_Name=" + userName);
        }
        ...
        protected void GeneratePDF(int days, int hourlySalary, string firstName, string lastName)
        {
            using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
            {
                ...
                para.Alignment = 0;
                document.Add(para);
                para = new Paragraph("ERPAplikacia s.r.o.                                   ");
                para.Alignment = 2;
                para.SpacingBefore = 3;
                para.SpacingAfter = 3;
                para.Font = FontFactory.GetFont(FontFactory.TIMES, 9, BaseColor.BLACK);
